...
merged and live-proven in the sandbox, with the per-environment sandbox lock
that models the real deploy locks. The real adapters must apply the same wait
to the real lock groups and must not rely on GitHub's status-filtered run
listing alone.
