...
Last reviewed: **2026-09-22**, against merged Coordinator `main` source
`4ae7425`, frontend `main` source `7471ac1`, backend `main` source `fe7f0cd`, the live package and consumer
...
| Operational monitoring intake       | Monitoring requests can be created, validated, displayed, labeled, have their exact PR checked, and enter the profile release sequence without an application-service catalog                                                                                                                                                                                                    | Merged intake came from [PR #72](https://github.com/6529-Collections/6529-release-coordinator/pull/72). Sandbox deployment has live proof below. The current working tree pins the existing real monitoring workflow and orders it after the product `main` merge and before production application deployments; this real path has offline tests but no live product acceptance or merge evidence yet.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Guarded inbox selection             | `RELEASE_COORDINATOR_SCOPE=filtered` exposes only repeated `--issue` values whose verified submitter matches `--actor`; `inbox` exposes the full selected-profile inbox. The resulting visible set then uses the normal Coordinator policy.                                                                                                                                      | Implemented in the current working tree with multi-ticket, actor-mismatch, database-isolation, CLI validation and resume coverage. The v7 journal saves the canonical filter so resume cannot widen it. The [September 22 sandbox acceptance](./testing/profile-scope-sandbox-2026-09-22.md) proved one-ticket selection, a three-ticket visible set whose database-changing member was deferred from the two-ticket batch, the database ticket alone, and explicit same-run resume. This remains unmerged and has not selected or changed a real-product ticket.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
with cheap, clear blockers. The current working tree implements that policy for
both profiles. Only the sandbox version has live acceptance; the real version has
not changed a product repository.

For this working tree, `npm run check` passed on September 22: 592 tests ran,
589 passed and the three explicitly Docker-only cases were skipped. Lint,
formatting, documentation links, workflow policy and packed-package checks also
passed. This is local verification, not GitHub CI, review, merge or live release
evidence.

## Next steps

Finish review and delivery of the current shared-engine change, then perform one
deliberately filtered real acceptance with explicitly chosen Issue numbers and
the verified submitter actor. Start with a small no-database-change staging
ticket, verify every saved product workflow/E2E identity, and only then expand to
a production ticket. Record live evidence separately; the offline suite is not
proof that a product environment changed correctly.
