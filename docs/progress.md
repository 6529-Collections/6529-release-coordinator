...
read. Any mismatch ends the attempt. Save it, keep the lane for a person, and
require explicit authorization plus fresh matching evidence for a new attempt;
never reuse the newer SHA. This closes the check-to-dispatch source gap inside
the product workflow. It does not create the adapter, authorize a real release,
or prove that a deployment occurred.
