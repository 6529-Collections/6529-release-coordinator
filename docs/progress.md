...
| Waiting for other workflow runs | Before each protected merge and each workflow dispatch, the sandbox adapter waits without a time limit until the pinned release workflow has no active run in the target repository, logs each check and saves the blocking runs | Implemented with offline coverage for the dispatch and merge paths, every active status, interruption, resume and state validation; see the [acceptance case](./merge-rehearsal-testing.md#release-sequence-acceptance). No live sandbox proof yet, and the sandbox workflow lock is still per operation. |
...
The wait for other active workflow runs before each merge and dispatch has
offline coverage only; its live sandbox proof and the per-environment sandbox
workflow lock that models the real deploy locks are the next sandbox steps.
