...
| Monitoring dispatched from a branch other than test main | **Passed offline.** The pinned runner reports a failed `monitoring:source` check and no installed template; the contract rejects a monitoring operation outside the prod stage. |
| Staging-only request with monitoring; monitoring-only request | **Passed offline.** A staging release adds no monitoring step and its completion says monitoring deploys with the production release; a monitoring-only sandbox request waits because the service stage needs the complete sample application. |
