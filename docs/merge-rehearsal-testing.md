...
accepts only `environment`, and deploys the run's `github.sha`. The corrected
test workflow now does the same. The correction passed local Coordinator checks
and Docker-backed sample checks on test-backend PR #139 and test-frontend PR #127;
both merged to test `main` and were published to test `1a-staging` through
backend PR #140 and frontend PR #128. The
[fresh success-path acceptance](./testing/monitoring-branch-contract-2026-09-23.md)
verified both monitoring branches and exact run commits, matching application
deploys, and staging/production E2E. The same
[acceptance record](./testing/monitoring-branch-contract-2026-09-23.md)
also covers separate controlled monitoring failures with staging-only and
production-then-staging restoration. Its first production-failure run exposed
an early automatic frontend staging E2E during undo, before the restored backend
API deploy finished. PR #219 adds a fresh frontend recovery deploy and a
backend-before-frontend timing guard. Fresh ticket #49 repeated the controlled
production failure and accepted that recovery path: the new frontend deploy and
linked E2E followed restored backend API, while the early automatic run was not
...
The corrected contract's success path now has fresh sandbox acceptance: staging
monitoring from test `1a-staging` before staging applications, production
monitoring from test `main` before production applications, both matching run
commits. Controlled failures/restoration in each environment also have fresh
acceptance in that record. Earlier live runs remain historical proof of the
older sequence, not proof of the corrected branch contract.
...
**Batch acceptance requirements, implemented in the sandbox stage September 10, 2026.** See [progress](./progress.md), the [original dated batch acceptance](./testing/batch-2026-09-10.md),
...
implementation also selects one verified database-changing ticket alone; it never
...
The test backend carries a sample `ops/monitoring` package that mirrors the
real backend's shape: `src/alarms.json` (bounded alarm definitions),
`src/deploy.json` (a controlled deployment switch, `fail_environment`), and
committed `monitoring-staging.json` / `monitoring-prod.json` inventories
generated from `src/config/deploy-services.json`. `npm run build` inside the
package verifies the committed inventory against the generated one and fails
when it is stale; `npm run generate` rewrites it. The backend `Sandbox check`
builds the package on every PR. Only those four candidate files may change in a
...
dispatch whose ref does not match the monitoring environment (`1a-staging` or
`main`), verifies the built template against its
...
steps used the prod release stage and test `main`. The published correction now
makes the runner require the branch matching the operation environment. The
product-workflow adapter has fresh success-path acceptance; the generic fallback
has not been retested for this correction. Restoration must redeploy monitoring
from each restored environment branch.
...
| Main/staging moves or contains unrelated changes               | **Passed offline and exercised live during setup.** Recheck the actual base before mutation; never overwrite refs or promote all of staging. A later adapter guard reads both environment refs immediately before workflow dispatch and again before accepting its result; that guard has offline regression coverage only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Another workflow run is active before a merge or dispatch      | **Passed offline and live September 18.** The adapter lists the pinned workflow's queued, waiting, pending, requested and in-progress runs in the target repository before each protected merge and each dispatch, reading both GitHub's newest page of runs and its per-status counts, waits without a time limit until both show nothing active, logs each check, saves the blocking runs on the step record, and dispatches only after the last check is quiet. An interrupted wait dispatches nothing and resume dispatches once. The [September 18 acceptance](./testing/sandbox-inflight-wait-2026-09-18.md) kept the backend workflow busy with throwaway runs: the first attempt exposed a false quiet from GitHub's lagging status-filtered listing and stopped before any dispatch; with the fixed two-source check the resumed run waited 74 checks (27 minutes) before its first dispatch, stopped cleanly on Ctrl-C mid-wait, resumed and pressed once when quiet, waited before a frontend merge as well, and completed all fourteen operations for ticket #34. The pinned sandbox workflow now holds one lock per environment (`sandbox-release-<environment>`, `cancel-in-progress: false`), like the real deploy workflows, so a second run for the same environment waits and a third arrival cancels the waiting one. The sandbox keys that lock on `fromJSON(inputs.operation_json).environment`, which the contract restricts to `staging` or `prod` before any dispatch, where the real workflows key on a declared environment input; a hand-made dispatch with unparseable JSON fails before any step runs. It was republished through backend [PR #94](https://github.com/6529-Collections/release-coordinator-test-backend/pull/94) and frontend [PR #86](https://github.com/6529-Collections/release-coordinator-test-frontend/pull/86), and those main histories were then merged into each `1a-staging`. |
...
| Sample monitoring deployed after the test-main merge           | **Passed offline and live September 17.** The [monitoring acceptance](./testing/sandbox-monitoring-2026-09-17.md) released ticket #30 (monitoring-only change) and ticket #33 (application plus monitoring change) through sixteen operations each. A production ticket with `operational_deployments: ["monitoring"]` runs `prod:monitoring:staging` and `prod:monitoring:prod` after `prod:integrate:backend` and before any production application deployment; each result must carry the installed template bound to the build manifest and GitHub's matching artifact record.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Monitoring deployment failure                                  | **Passed offline and live September 17.** Ticket #32 in the same record failed at `prod:monitoring:staging` with no production application deployment started, then restoration redeployed monitoring from the restored commit and reran every check. The controlled `fail_environment` switch fails only its environment; the release stops before any production application deployment, restoration reverts test `main` and staging, and the restored monitoring is redeployed from the restored commit.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Legacy monitoring branch rejection                             | **Historical offline proof only.** The earlier sandbox contract rejected monitoring outside the prod stage and test `main`. The corrected branch-aligned contract supersedes that requirement; [September 23 acceptance](./testing/monitoring-branch-contract-2026-09-23.md) verified staging from `1a-staging` and prod from `main`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Legacy staging-only/monitoring-only requests                   | **Historical offline proof only.** The earlier staging release added no monitoring step; the corrected contract adds staging monitoring when selected. A monitoring-only sandbox request still waits because the service stage needs the complete sample application.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Database change/unknown, conflicting revert or failed recovery | **Database-changing multi-ticket batches remain unsupported; failed restoration has local coverage.** Missing, moved or incompatible environment evidence stops automatic work for a person. Real-product rollback and live restoration-failure acceptance remain future work.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Real product workflow adapters                                 | **Merged in PRs #218 and #219 with offline tests; branch-aligned monitoring passed sandbox success and recovery acceptance.** Existing frontend/backend Actions are pinned and results are bound to exact source/environment. No real-product run has exercised the adapter.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
