...
| Sample monitoring deployed after the test-main merge | **Passed offline.** A production ticket with `operational_deployments: ["monitoring"]` runs `prod:monitoring:staging` and `prod:monitoring:prod` after `prod:integrate:backend` and before any production application deployment; each result must carry the installed template bound to the build manifest and GitHub's matching artifact record. |
| Monitoring deployment failure | **Passed offline.** The controlled `fail_environment` switch fails only its environment; the release stops before any production application deployment, restoration reverts test `main` and staging, and the restored monitoring is redeployed from the restored commit. |
