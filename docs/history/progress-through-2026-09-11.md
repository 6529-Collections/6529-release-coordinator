# Progress evidence through September 11, 2026

Historical snapshot of `docs/progress.md` from source **`e6537ab`**, saved during
the September 11 documentation cleanup. The previous text is preserved below;
only relative file links were adjusted for this location.

Statements such as “current,” “next,” “not pushed” and older test counts describe
their original checkpoints. They are not current work instructions or new live
verification. Use [current progress](../progress.md) for present scope and next
steps, and the linked dated acceptance records for their specific proof.
...
[one historical record](npm-migration.md). Historical findings and
