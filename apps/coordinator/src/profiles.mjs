...
const profile = (name, inbox, repositories) =>
  Object.freeze({
    ...
    branch: "main",
    rehearsal_destinations: Object.freeze({
      frontend: "main",
      backend: "main"
    }),
...
export const sandboxProfile = profile(
  "sandbox",
  repository("6529-Collections/release-coordinator-test-inbox", 1362580376),
  {
    frontend: repository(
...
export function selectProfile(value, { defaultReal = false } = {}) {
  ...
  throw new Error(
    "Set RELEASE_COORDINATOR_PROFILE to sandbox or real. Unknown profiles never fall back."
  );
}

export function repositoryRole(name, selected = realProfile) {
  const role = Object.keys(selected.repositories).find(
    (key) => selected.repositories[key].full_name === `6529-Collections/${name}`
  );
  if (!role)
    throw new Error("Request repository is outside the selected profile.");
  return role;
}
...
export function canonicalRequest(request, selected = realProfile) {
  ...
  if (selected.name === "sandbox") {
    if (value?.profile !== "sandbox")
      throw new Error("Sandbox requests require profile: sandbox.");
    delete value.profile;
  }
  if (!Array.isArray(value?.release_parts))
    throw new Error("Request must contain release parts.");
...
export function validateProfileRequest(request, selected = realProfile) {
  try {
    return validateReleaseRequest(canonicalRequest(request, selected));
  } catch (error) {
    return {
      ok: false,
      errors: [
        { code: "profile_request", location: "$", message: error.message }
      ]
    };
  }
}
