...
export function createReadinessGitHub({
  ...
  async function api(method, endpoint, fields = []) {
    ...
    try {
      ...
      throw new Error(
        "GitHub readiness read failed. Check gh authentication, repository read access, and network availability."
      );
    }
    let result;
    try {
    ...
    if (!result || result.errors)
      throw new Error("GitHub could not return complete readiness evidence.");
    return result;
  }
  ...
  return {
    async pullRequest(repository, number) {
      ...
      for (;;) {
        ...
        if (
          ...
          throw new Error(
            "GitHub did not return checks for the PR's exact head."
          );
        }
        const { commits: ignored, ...metadata } = pr;
        if (snapshot && JSON.stringify(snapshot) !== JSON.stringify(metadata)) {
          throw new Error(
            "PR state changed while reading check pages. Run the scan again."
          );
        }
        snapshot = metadata;
        ...
        for (const check of connection.nodes) {
          if (
            ...
            throw new Error(
              "GitHub returned missing, repeated, or unknown PR checks."
            );
          }
          ids.add(check.id);
          checks.push(check);
        ...
        if (typeof cursor !== "string" || !cursor || cursors.has(cursor)) {
          throw new Error(
            "GitHub returned a missing or repeated check-page cursor."
          );
        }
        cursors.add(cursor);
      }
    ...
    async catalog(commit) {
      ...
      const file = await api(
        "GET",
        `repos/${profile.repositories.backend.full_name}/contents/${catalogPath}?ref=${commit}`
      );
      if (
        file.type !== "file" ||
        ...
        !shaPattern.test(file.sha)
      ) {
        throw new Error(
          "GitHub did not return the backend catalog file at the requested commit."
        );
      }
      let catalog;
      try {
