...
export async function runInboxRunCli(args, { client, get, github, env = process.env, run = processInbox,
  plan = generateInboxPlan, rehearse = rehearseInboxTicket, signal,
  ...
  if (options.closeTest && !options.issueNumber
    || options.resume && (options.issueNumber || options.closeTest)) { stderr(help); return 2; }
  let profile;
  try {
    ...
    await get.identity?.();
    const report = await run({ ...options, signal, profile, api: client.request, identity: client.identity, get, github,
      plan: entry => plan(entry, { profile, signal }),
      rehearsal: (entry, plan) => rehearse(entry, plan, { profile, get, signal }) });
    stdout(seen.has("--json") ? `${JSON.stringify(report, null, 2)}\n`
      : `Inbox run ${report.run_id}; no release authorized.\n${report.requests.map(item =>
        `#${item.issue_number}: ${item.status}; ${item.applied ? "verified" : "left unchanged"}; rehearsal ${item.rehearsal?.status ?? "not-run"}${item.reasons?.length ? `; ${item.reasons.join(", ")}` : ""}${item.rehearsal?.report_file ? `\nReport: ${item.rehearsal.report_file}` : ""}${item.assignment === "unavailable" ? "; submitter assignment unavailable; see status comment for lookup" : ""}`).join("\n")}\n`);
    if (report.requests.some(item => !item.applied || item.rehearsal?.status === "unknown")) return 2;
    if (report.requests.some(item => item.rehearsal?.status === "stale")) return 3;
    return report.requests.some(item => !["closed", "completed"].includes(item.status) && item.rehearsal?.status !== "passed") ? 1 : 0;
  } catch (error) {
