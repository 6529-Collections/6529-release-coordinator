...
function allowedPath(path, prefix) {
  ...
  return (
    /^\/issues\?state=(?:open|all)&labels=release-request&sort=created&direction=asc&per_page=100&page=[1-9][0-9]*$/u.test(
      relative
    ) ||
    /^\/actions\/runs\/[1-9][0-9]*$/u.test(relative) ||
    /^\/actions\/runs\/[1-9][0-9]*\/attempts\/[1-9][0-9]*\/jobs\?per_page=100&page=[1-9][0-9]*$/u.test(
      relative
    ) ||
...
export function createGitHubReader({
  ...
  async function read(path) {
    ...
    try {
      ...
      if (error.code === "ENOENT") {
        throw new Error(
          "GitHub CLI (gh) is missing. Install it and authenticate to github.com.",
          { cause: error }
        );
      }
      throw new Error(
  ...
  const get = async (path) => {
    if (!allowedPath(path, prefix))
      throw new Error(
        "The inbox reader refused an unsupported GitHub endpoint."
      );
    return read(path);
  };
  get.identity = async () => {
    ...
    if (
      repo.id !== profile.inbox.id ||
      repo.full_name !== profile.inbox.full_name ||
      repo.private !== profile.inbox.private
    ) {
      throw new Error(
        "Inbox repository identity or visibility does not match the selected profile."
      );
    }
    return repo;
  };
