...
const statusOf = (checks) =>
  checks.some((item) => item.status === "blocked") ? "blocked" : "unknown";

export function validatePull(
  pr,
  repository,
  number,
  fullName = `6529-Collections/${repository}`
) {
  if (
    !pr ||
    pr.number !== number ||
    pr.repository?.nameWithOwner !== fullName ||
    typeof pr.headRefOid !== "string" ||
    !sha.test(pr.headRefOid) ||
    typeof pr.baseRefOid !== "string" ||
    !sha.test(pr.baseRefOid) ||
...
function requiredCheckStatus(item) {
  if (item.__typename === "StatusContext") {
    ...
    if (["FAILURE", "ERROR", "PENDING", "EXPECTED"].includes(item.state))
      return "blocked";
    return "unknown";
  }
  ...
  if (["SUCCESS", "NEUTRAL", "SKIPPED"].includes(item.conclusion))
    return "pass";
  if (
    [
      "FAILURE",
...
export function inspectPull(
  ...
  checks.push(
    check(
      ...
      exact ? "pass" : "blocked",
      exact
        ? "The PR still names the requested branch and exact commit."
        : "The request is outdated: the PR branch or commit differs. This does not prove cancellation or replacement.",
      {
  ...
  checks.push(
    check(
      ...
      pr.headRepository?.nameWithOwner === fullName
        ? "PR source is the named product repository."
        : "A deleted or fork source cannot be bound to this request's repository with the available evidence."
    )
  );
  checks.push(
    check(
      ...
      pr.state === "CLOSED" || pr.isDraft
        ...
        : pr.state === "MERGED"
          ...
          : "pass",
      pr.state === "MERGED"
        ? "PR is already merged. That is not proof of deployment; a further release needs target and history evidence."
        : pr.state === "CLOSED"
          ? "PR is closed without merging."
          : pr.isDraft
            ? "PR is still a draft."
            : "PR is open and is not a draft.",
      ...
      !open
        ...
        : pr.mergeable === "CONFLICTING"
          ...
          : pr.mergeable === "MERGEABLE"
            ...
            : "unknown",
      !open
        ? "Current-base mergeability is not usable for this closed, merged, or draft PR."
        : pr.mergeable === "CONFLICTING"
          ? "GitHub reports conflicts with this PR's base branch."
          : pr.mergeable === "MERGEABLE"
            ? "GitHub reports no conflicts with this PR's base branch."
  ...
  checks.push(
    check(
      ...
      {
        required,
        optional_check_count: pr.checks.filter(
...
export function fingerprint(pr) {
  return JSON.stringify({
    ...pr,
    checks: [...pr.checks].sort((a, b) => a.id.localeCompare(b.id))
  });
}
...
async function inspectCatalogs(request, github, checks) {
  ...
  try {
    ...
    for (const commit of commits) {
      ...
      if (source.commit !== commit || !sha.test(source.blob_sha))
        throw new Error(
          "Catalog evidence does not match the requested commit."
        );
      ...
      if (signature !== undefined && signature !== currentSignature) {
        checks.push(
          check(
            "backend_services",
            "unknown",
            "Requested backend commits contain different service definitions. A catalog from the exact combined merge result is needed.",
            evidence
          )
        );
        return;
      ...
      check(
        ...
        graph.errors.length
          ...
          : graph.missing_prerequisites.length
            ? "Catalog prerequisites are not selected. Their required deployed state is unverified; no services were added automatically."
...
export async function inspectReadiness(
  ...
  const result = {
    ...
    target: entry.request?.target ?? null,
    status: "unknown",
  ...
  if (
    ...
    checks.push(
      check(
        "saved_record",
        entry.status === "invalid" ? "blocked" : "unknown",
        "Saved request proof did not pass; no product readiness reads were attempted.",
        { errors: entry.errors }
      )
    );
  ...
  checks.push(
    check(
      "saved_record",
      "pass",
      "Saved request matches its submission workflow proof."
    )
  );
  ...
  checks.push(
    check(
      ...
      graph.status,
      graph.errors.join(" ") ||
        "Release-part IDs and PRs are unique; dependencies exist and contain no cycle.",
  ...
  for (const part of request.release_parts) {
    for (const requested of part.pull_requests) {
      const item = {
        ...
        url: `https://github.com/6529-Collections/${part.repository}/pull/${requested.number}`,
        checks: []
      };
  ...
  if (graph.status === "pass")
    await inspectCatalogs(canonicalRequest(request, profile), github, checks);
  else
    checks.push(
      check(
        "backend_services",
        "unknown",
        "Service ordering was not evaluated because the release-part graph is invalid."
      )
    );
    ...
    try {
      ...
      item.checks.push(
        check(
          ...
          changed
            ? "PR, base, checks, or review state changed during this scan. Run it again; this is not a stable observation."
      ...
      item.checks.push(
        check(
          "observation_stability",
          "unknown",
          `Final PR recheck failed. ${error.message}`
        )
      );
    }
  }
  ...
  result.status = statusOf([
    ...checks,
    ...result.pull_requests.flatMap((item) => item.checks)
  ]);
  return result;
}
...
export async function checkReadiness({
  ...
  for (let index = 0; index < inbox.requests.length; index += 1) {
    ...
    if (overlaps.length)
      requests[index].checks.push(
        check(
          "overlapping_requests",
          "unknown",
          "Other pending requests name the same PR and target. No request was chosen or marked replaced.",
          { issue_numbers: overlaps }
        )
      );
  }
  return {
    ...
    inbox_checked_at: inbox.checked_at,
    release_authorized: false,
    counts: {
      pending: requests.length,
      blocked: requests.filter((item) => item.status === "blocked").length,
      unknown: requests.filter((item) => item.status === "unknown").length
    },
    requests
  };
}
...
export function formatReadiness(report) {
  const lines = [
    "Release readiness observations (read-only; never permission to release)",
  ...
  const add = (item, indent) => {
    lines.push(
      `${indent}${item.status.toUpperCase()} ${item.id}: ${safe(item.message)}`
    );
    if (item.evidence !== undefined)
      lines.push(`${indent}  Evidence: ${safe(JSON.stringify(item.evidence))}`);
  };
  for (const entry of report.requests) {
    lines.push(
      "",
      `#${entry.issue_number} — ${entry.status.toUpperCase()} — ${safe(entry.request_id)} — ${safe(entry.target)}`,
      entry.issue_url
    );
  ...
  if (!report.requests.length)
    lines.push(
      "No pending requests observed. This is not release authorization."
    );
  return `${lines.join("\n")}\n`;
}
