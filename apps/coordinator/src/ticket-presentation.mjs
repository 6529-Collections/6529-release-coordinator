...
const prose = text => clean(text).replace(/[\\`*_{}\[\]<>#|]/gu, "\\$&").replaceAll("@", "＠");
...
export async function ensureLabels(api, names) {
  for (const name of names.filter(value => managedLabels.has(value))) {
    const found = await api({ method: "GET", path: `/labels/${encodeURIComponent(name)}` });
    if (found.status === 200) continue;
    if (found.status !== 404) throw new Error(`Could not inspect label ${name}.`);
    const color = name.startsWith("reason:") ? "fbca04" : name.startsWith("status:") ? "5319e7" : "0969da";
    const created = await api({ method: "POST", path: "/labels", body: { name, color } });
    if (created.status === 201) continue;
    if (created.status === 422 && (await api({ method: "GET", path: `/labels/${encodeURIComponent(name)}` })).status === 200) continue;
    throw new Error(`Could not create label ${name}.`);
  }
}

export async function response(api, method, path, body, expected = 200) {
  const result = await api({ method, path, ...(body === undefined ? {} : { body }) });
  if (result.status !== expected) throw new Error(`GitHub ${method} ${path} failed (HTTP ${result.status}).`);
  return result.data;
}
...
export async function comments(api, number) {
  ...
  for (let page = 1; ; page++) {
    const batch = await response(api, "GET", `/issues/${number}/comments?per_page=100&page=${page}`);
    if (!Array.isArray(batch)) throw new Error("Incomplete comment list.");
    for (const item of batch) {
      if (!Number.isSafeInteger(item.id) || seen.has(item.id)) throw new Error("Invalid or repeated comment identity.");
      all.push(item); seen.add(item.id);
    }
...
export function receivedDecision() {
  return { status: "received", reasons: [], next_action: "Inspect this request on the next inbox processing run.",
    action_owner: "Coordinator", submitter_action: "None currently required." };
}

export function statusComment({ decision, actor, at, submitter, request, number, marker, assignment }) {
  const lines = [`<!-- 6529-coordinator-status:${marker} -->`, "## Coordinator status", "",
    `**Status:** ${decision.status}`, "",
    `**Submitter:** ${submitter ? `${prose(submitter.login)} (GitHub ID ${submitter.id})` : "Identity not yet verified."}`,
    `**Next action:** ${prose(decision.next_action)}`, `**Action owner:** ${prose(decision.action_owner)}`,
    `**Submitter action:** ${prose(decision.submitter_action)}`, "", "**Why:**"];
  if (!decision.reasons.length) lines.push("- Request received; inspection is pending.");
  ...
  if (request) {
    ...
    for (const part of request.release_parts) {
      for (const pr of part.pull_requests) lines.push(`- https://github.com/6529-Collections/${part.repository}/pull/${pr.number} — commit \`${pr.commit}\``);
      if (part.deploy_units) lines.push(`- Backend scope: ${prose(part.deploy_units.join(", "))}`);
    }
  }
  if (assignment === "unavailable") lines.push("", "**Assignment:** GitHub did not assign the verified submitter. The accepted request is preserved.",
    `Lookup: run \`npm run inbox:read -- --submitter ${submitter.login}\` and find #${number}.`);
  lines.push("", `**Last meaningful decision:** ${at}; ${prose(actor.login)} (GitHub ID ${actor.id}).`,
    "", "This ticket status does not authorize a merge or deployment.");
  const body = lines.join("\n");
  if (body.length > 60_000) throw new Error("Status explanation exceeds the GitHub comment limit.");
  return body;
}

export async function assignSubmitter(api, issue, submitter) {
  if (!submitter) return "unverified";
  if (issue.assignees?.some(user => String(user.id) === submitter.id)) return "assigned";
  // Resolve by stable ID: a renamed/recycled login must not assign another user.
  let eligible = false;
  for (let page = 1; ; page++) {
    const result = await api({ method: "GET", path: `/assignees?per_page=100&page=${page}` });
    if (result.status !== 200 || !Array.isArray(result.data)) return "unavailable";
    if (result.data.some(user => user.login === submitter.login && String(user.id) === submitter.id)) eligible = true;
    if (result.data.length < 100) break;
  }
  if (!eligible) return "unavailable";
  const assigned = await api({ method: "POST", path: `/issues/${issue.number}/assignees`, body: { assignees: [submitter.login] } });
  if (assigned.status !== 201) return "unavailable";
  return assigned.data.assignees?.some(value => String(value.id) === submitter.id) ? "assigned" : "unavailable";
}
...
export async function initializeTicket({ api, issue, request, submitter, at, writer }) {
  const marker = randomUUID();
  const presentation = { marker, author_id: String(writer.id), comment_id: null, warnings: [] };
  try {
    const decision = receivedDecision();
    const labels = desiredLabels(issue, decision, request);
    await ensureLabels(api, labels);
    await response(api, "PATCH", `/issues/${issue.number}`, { title: ticketTitle(request), labels });
    const assignment = await assignSubmitter(api, issue, submitter);
    if (assignment !== "assigned") presentation.warnings.push("Submitter assignment unavailable; lookup remains available.");
    const body = statusComment({ decision, actor: writer, at, submitter, request, number: issue.number, marker, assignment });
    const comment = await response(api, "POST", `/issues/${issue.number}/comments`, { body }, 201);
    if (String(comment.user?.id) !== presentation.author_id || !Number.isSafeInteger(comment.id)) throw new Error("Unexpected status comment author or identity.");
...
export async function saveOrganizedReleaseRequestIssue(options) {
  ...
  try {
    const issue = await response(options.githubRequest, "GET", `/issues/${inbox.issue.number}`);
    inbox.presentation = await initializeTicket({ api: options.githubRequest, issue, request: options.request,
      submitter: { login: options.actor, id: String(options.actorId) }, at: options.submittedAt,
      writer: { login: issue.user.login, id: String(issue.user.id) } });
  } catch {
    inbox.presentation = { warnings: ["Request accepted; initial presentation needs repair by the processor."] };
  }
  return inbox;
}
