...
export async function dispatchRequest(
  ...
  if (!/^HTTP\/\S+ 204\b/u.test(output))
    throw new Error(
      "Intake dispatch did not return a confirmed acceptance; inspect the inbox before retrying."
    );
}
...
export async function submitProfileRequest(
  ...
  if (!validateProfileRequest(request, profile).ok)
    throw new Error("Request does not match the selected profile's schema.");
  await get.identity?.();
  const checksum = releaseRequestChecksum(request);
  const inspect = async () => {
    const report = await readInbox({ get, profile, includeClosed: true });
    const matches = report.requests.filter(
      (entry) => entry.request?.request_id === request.request_id
    );
    if (
      matches.length > 1 ||
      matches.some(
        (entry) => releaseRequestChecksum(entry.request) !== checksum
      )
    )
      throw new Error(
        "Request identity already exists with different or duplicated data; no dispatch is safe."
      );
    return matches[0];
  };
  ...
  for (;;) {
    if (entry?.status === "valid")
      return {
        ...
        github_actor: entry.github_actor,
        workflow: entry.workflow,
        release_authorized: false
      };
    if (entry?.status === "invalid")
      throw new Error(
        "The matching ticket failed receipt verification; inspect the inbox before retrying."
      );
    if (now() >= deadline)
      throw new Error(
        "Receipt is not yet verified; intake may still be running. Inspect the saved request and inbox before retrying."
      );
    await pause(3000);
    entry = await inspect();
  }
...
export async function runSubmissionCli(
  args,
  {
    env = process.env,
    load = readRehearsalManifest,
    submit = submitProfileRequest,
    save = saveProfileRecord,
    root = process.cwd(),
  ...
  try {
    ...
    stdout(
      json
        ? `${JSON.stringify({ ...result, record_file: record }, null, 2)}\n`
    ...
    if (prepared) {
      try {
        await save(profile, `${attempt}.result.json`, failure, root);
      } catch (recordError) {
        failure.result_record_error = recordError.message;
      }
    }
