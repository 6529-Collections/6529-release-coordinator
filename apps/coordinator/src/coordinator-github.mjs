...
const keys = (body, allowed) =>
  body && Object.keys(body).every((key) => allowed.includes(key));

function allowed(method, path, body) {
  if (method === "GET" && body === undefined)
    return (
      path === "" ||
      /^\/issues\/[1-9][0-9]*$/u.test(path) ||
      /^\/issues\/[1-9][0-9]*\/comments\?per_page=100&page=[1-9][0-9]*$/u.test(
        path
      ) ||
  ...
  if (method === "POST" && path === "/labels")
    return (
      keys(body, ["name", "color"]) &&
      managedLabels.has(body.name) &&
      /^[0-9a-f]{6}$/u.test(body.color)
    );
  if (method === "POST" && /^\/issues\/[1-9][0-9]*\/labels$/u.test(path))
    return (
      keys(body, ["labels"]) &&
      Array.isArray(body.labels) &&
      body.labels.every((label) => managedLabels.has(label))
    );
  if (
    method === "DELETE" &&
    /^\/issues\/[1-9][0-9]*\/labels\/[^/?]+$/u.test(path)
  )
    return (
      body === undefined &&
      managedLabels.has(decodeURIComponent(path.split("/").at(-1)))
    );
  if (method === "PATCH" && /^\/issues\/[1-9][0-9]*$/u.test(path))
    return (
      keys(body, ["title", "labels", "state", "state_reason"]) &&
      ...
      (body.state_reason === undefined ||
        ["completed", "not_planned", "reopened"].includes(body.state_reason))
    );
  if (
    (method === "POST" && /^\/issues\/[1-9][0-9]*\/comments$/u.test(path)) ||
    (method === "PATCH" && /^\/issues\/comments\/[1-9][0-9]*$/u.test(path))
  )
    return (
      keys(body, ["body"]) &&
  ...
  if (method === "POST" && /^\/issues\/[1-9][0-9]*\/assignees$/u.test(path))
    return (
      keys(body, ["assignees"]) &&
      Array.isArray(body.assignees) &&
      body.assignees.length === 1 &&
      /^[a-zA-Z0-9-]+$/u.test(body.assignees[0])
    );
  ...
  if (method === "POST" && path === "/git/commits")
    return (
      ...
      body.message.startsWith("Inbox journal: ") &&
      Array.isArray(body.parents) &&
      body.parents.length <= 1 &&
      body.parents.every(sha)
    );
  if (method === "POST" && path === "/git/refs")
    return (
      keys(body, ["ref", "sha"]) &&
      body.ref === `refs/heads/${stateBranch}` &&
      sha(body.sha)
    );
  return (
    method === "PATCH" &&
...
export function createCoordinatorGitHub({
  ...
  return {
    request: async ({ method, path, body }) => {
      if (!allowed(method, path, body))
        throw new Error("Coordinator refused an unsupported GitHub operation.");
    ...
    identity: async () => {
      ...
      if (
        user.status !== 200 ||
        !Number.isSafeInteger(user.data?.id) ||
        !/^[a-zA-Z0-9-]+$/u.test(user.data?.login) ||
        repo.status !== 200 ||
        repo.data?.full_name !== profile.inbox.full_name ||
        repo.data?.id !== profile.inbox.id ||
        repo.data?.private !== profile.inbox.private ||
        repo.data?.permissions?.push !== true
      ) {
        throw new Error(
          "Processing requires an authenticated repository writer with Issue and contents write access."
        );
      }
