...
if (
  ...
  (!resume && process.argv[2] !== "--seed-empty-sandbox-repos")
)
  throw new Error(
    "Explicit --seed-empty-sandbox-repos or --resume-recorded-sandbox-setup is required. This creates test branches and PRs."
  );
const root = fileURLToPath(new URL("../../../", import.meta.url));
const directory = path.join(
  root,
  ".release-coordinator",
  "rehearsal-setup-20260909"
);
...
async function gh(endpoint, method = "GET", body) {
  ...
  return JSON.parse(
    (await exec("gh", args, { maxBuffer: 4 * 1024 * 1024 })).stdout
  );
}
...
for (const [role, identity] of Object.entries(sandboxRepositories)) {
  ...
  if (
    ...
    remote.private !== identity.private ||
    remote.permissions?.admin !== true
  )
    throw new Error(
      `Refusing to seed ${role}: identity, visibility, or admin access does not match the pinned repository.`
    );
  ...
  if (prior) {
    if (
      !resume ||
      prior.id !== identity.id ||
      prior.full_name !== identity.full_name ||
      branches.find((b) => b.name === "main")?.commit.sha !== prior.base ||
      branches.find((b) => b.name === "rehearsal-target")?.commit.sha !==
        prior.target
    )
      throw new Error(
        `Refusing to resume ${role}: recorded baseline identity changed.`
      );
  } else if (branches.length || remote.size !== 0)
    throw new Error(`Refusing to overwrite existing ${role} branches.`);
...
for (const { role, identity } of roles) {
  ...
  const env = {
    ...
    GIT_AUTHOR_NAME: "Coordinator sandbox",
    GIT_AUTHOR_EMAIL: "rehearsal@example.invalid",
    GIT_COMMITTER_NAME: "Coordinator sandbox",
  ...
  if (prior) {
    base = prior.base;
    if (
      (await git(["rev-parse", "refs/heads/main"])) !== base ||
      (await git(["rev-parse", "refs/heads/rehearsal-target"])) !== prior.target
    )
      throw new Error(
        "Local seed branches differ from recorded fixture evidence."
      );
    seed.repositories[role] = { ...prior, ...identity };
  } else {
    await git(["init", "-b", "main"]);
    await files({
      "README.md": `# Coordinator test ${role}\n\nSample code for merge rehearsals. No deployments or real release requests.\n`,
      "shared.txt": "original\n",
      ...
      ...(role === "backend"
        ? { "src/config/deploy-services.json": json(catalog()) }
        : {})
    });
    await git(["add", "."]);
    await git(["commit", "-m", "Seed isolated Coordinator rehearsal fixtures"]);
    ...
    await gh(`repos/${identity.full_name}`, "PATCH", {
      default_branch: "main"
    });
    await git(["checkout", "-b", "rehearsal-target", base]);
    await files({ "shared.txt": "destination change\n" });
    await git(["add", "."]);
    await git(["commit", "-m", "Create alternative test destination"]);
  ...
  const protection = {
    required_status_checks: {
      strict: false,
      checks: [{ context: "Sandbox check", app_id: 15368 }]
    },
    enforce_admins: true,
  ...
  for (const branch of ["main", "rehearsal-target"]) {
    ...
    try {
      const value = await gh(
        `repos/${identity.full_name}/branches/${branch}/protection`,
        "PUT",
        protection
      );
      if (
        ...
        !value.required_status_checks?.checks?.some(
          (check) => check.context === "Sandbox check" && check.app_id === 15368
        )
      )
        throw new Error("Required sandbox protections were not confirmed.");
      ...
      process.stdout.write(
        `${role}/${branch}: required-check protection unavailable on current private-repository plan.\n`
      );
    }
  ...
  const scenarios =
    role === "frontend"
      ? [
          ...
          [
            "ci-failure",
            { FAIL_CHECK: "deliberately fail the required check\n" }
          ],
  ...
  for (const [name, values, state] of scenarios) {
    ...
    if (saved) {
      const existing = await gh(
        `repos/${identity.full_name}/pulls/${saved.number}`
      );
      if (
        existing.head.sha !== (saved.current_commit ?? saved.commit) ||
        existing.head.ref !== saved.branch
      )
        throw new Error("Existing fixture PR differs from the saved seed.");
      continue;
    }
    ...
    await files(values);
    await git(["add", "."]);
    await git(["commit", "-m", `Sandbox scenario: ${name}`]);
    ...
    seed.repositories[role].pulls[name] = {
      number: pr.number,
      branch,
      commit,
      url: pr.html_url
    ...
    await writeFile(path.join(directory, "seed.json"), json(seed));
    if (state === "closed")
      await gh(`repos/${identity.full_name}/pulls/${pr.number}`, "PATCH", {
        state: "closed"
      });
