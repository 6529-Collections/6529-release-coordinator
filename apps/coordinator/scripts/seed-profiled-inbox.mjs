...
if (
  ...
  process.argv[2] !== "--seed-empty-inbox" ||
  process.argv[3] !== "--coordinator-ref" ||
  !/^[0-9a-f]{40}$/u.test(process.argv[4])
)
  throw new Error(
    "Usage: --seed-empty-inbox --coordinator-ref FULL_COMMIT. Creates fixture main and branch protection."
  );
const source = process.argv[4],
  inbox = sandboxProfile.inbox;
...
if (
  repo.id !== inbox.id ||
  repo.private !== inbox.private ||
  repo.permissions?.admin !== true ||
  (await gh(`repos/${inbox.full_name}/branches`)).length ||
  repo.size !== 0
)
  throw new Error("Refusing to seed a nonempty, unowned, or unexpected inbox.");
if (
  (await gh(`repos/${realProfile.inbox.full_name}/commits/${source}`)).sha !==
  source
)
  throw new Error("Coordinator source commit is not available on GitHub.");
...
try {
  ...
  await git([
    "commit",
    "-m",
    "Seed isolated inbox with pinned shared Coordinator intake"
  ]);
  ...
  await git([
    ...
    "add",
    "origin",
    `git@github.com:${inbox.full_name}.git`
  ]);
  ...
  await exec("gh", [
    ...
    "--method",
    "PUT",
    `repos/${inbox.full_name}/branches/main/protection`,
    "--input",
    bodyFile
  ]);
  ...
  if (
    ...
    throw new Error("Inbox main protection was not verified.");
  process.stdout.write(
    `${JSON.stringify({ inbox, main_commit: commit, coordinator_source: source, protection_verified: true }, null, 2)}\n`
  );
} finally {
