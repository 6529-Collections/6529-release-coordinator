...
const root = path.resolve(`.release-coordinator/${mode}-development`);
...
// Monitoring cases start from test main after the sample monitoring package
// was published there. Each backend change has a frontend companion so the
// sandbox ticket stays a complete backend/frontend sample.
const monitoringCases = {
  only_backend: {
    role: "backend",
    files: monitoringFiles({
      alarms: [
        { metric: "Errors", threshold: 2 },
        { metric: "Throttles", threshold: 5 },
        { metric: "Duration", threshold: 300 }
      ]
    })
  },
  only_frontend: {
  ...
  mixed_frontend: {
    role: "frontend",
    files: { "docs/monitoring-mixed.md": "Mixed monitoring companion.\n" }
  },
  fail_backend: {
    role: "backend",
    files: monitoringFiles({ failEnvironment: "staging" })
  },
  fail_frontend: {
    role: "frontend",
