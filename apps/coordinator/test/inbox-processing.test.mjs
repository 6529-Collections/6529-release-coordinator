...
test("new intake creates readable scope, received status, verified assignment, and one separate comment", async () => {
  ...
  for (const state of ["open", "closed"]) {
    issue.state = state;
    issue.labels = ["release-request", "status:closed", "reason:test"];
    const before = issueWrites(f).length;
    const retry = await saveOrganizedReleaseRequestIssue(intake(f));
    assert.equal(retry.issue.created, false);
    assert.equal(issue.state, state);
    assert.equal(issueWrites(f).length, before);
    assert.equal(f.comments.length, 1);
    assert.equal(issue.body, original);
  }
  f.request.database_change = "yes";
  await assert.rejects(
    saveOrganizedReleaseRequestIssue(intake(f)),
    /different|checksum/iu
  );
});
...
test("processing legacy intake preserves receipt and unrelated labels; waiting stays readable and unchanged retry writes no tickets", async () => {
  ...
  assert.equal(first.requests[0].status, "waiting");
  assert.equal(first.release_authorized, false);
  assert.equal(f.issue.body, body);
  assert.equal(f.issue.state, "open");
  assert.ok(f.issue.labels.includes("user-note"));
  assert.ok(!f.issue.labels.includes("pending"));
  assert.deepEqual(
    f.issue.labels.filter((value) => value.startsWith("status:")),
    ["status:waiting"]
  );
  assert.equal((await readInbox(f)).counts.pending, 1);
  assert.equal(f.comments.length, 1);
  const before = issueWrites(f).length,
    transition = f.state().tickets[1].transitions[0].id;
...
test("processor adopts only the exact comment identity proven by the intake workflow", async () => {
  ...
  f.issues.length = 0;
  const saved = await saveOrganizedReleaseRequestIssue(intake(f));
  f.result.inbox_presentation = saved.presentation;
  f.comments.push({
    id: 8000,
    issue_number: 1,
    user: { id: 999 },
  ...
  assert.equal(f.state().tickets[1].comment.id, saved.presentation.comment_id);
  assert.match(f.comments[0].body, /Status:\*\* waiting/);
  assert.match(f.comments[1].body, /Status:\*\* received/);
});
...
test("moving PR evidence never becomes automatic closure", async () => {
  ...
  f.github.pullRequest = async () => ({
    ...
    headRefOid: (++calls % 2 ? "e" : "f").repeat(40)
  });
  await processOne(f);
  assert.equal(f.issue.state, "open");
...
test("closure evidence is rechecked after journaling; changed facts leave an explicit partial run", async () => {
  ...
  f.pr.headRefOid = "e".repeat(40);
  f.after = async (call) => {
    if (call.method === "POST" && call.path === "/issues/1/comments")
      f.pr.headRefOid = "a".repeat(40);
  };
  await assert.rejects(processOne(f), /evidence changed before closure/);
  assert.equal(f.issue.state, "open");
  assert.ok(f.state().lock);
  assert.equal(f.state().tickets[1].applied, null);
  f.after = async () => {};
...
for (const [name, change, status] of [
  ...
  test(`${name} stays visible without product reads, assignment, or closure`, async () => {
    ...
    assert.equal(f.issue.assignees.length, 0);
    assert.equal(f.productCalls.length, 0);
    assert.match(f.comments[0].body, /Coordinator maintainers/);
  });
...
test("already merged code closes without claiming deployment and preserves its receipt on retry and resubmission", async () => {
  ...
  assert.match(
    f.comments[0].body,
    /same merged PR again will lead to the same closure/
  );
  assert.match(f.comments[0].body, /aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/);
  ...
  assert.equal(f.comments.length, 1);
  assert.equal(f.state().tickets[1].transitions.length, 1);
  assert.equal(f.state().tickets[1].applied, first.requests[0].transition_id);
  assert.equal(f.state().lock, null);
});
...
test("a waiting request later found merged closes using the same comment and keeps the previous decision", async () => {
  const f = fixture();
  await processOne(f);
  const previous = structuredClone(f.state().tickets[1].transitions[0]),
...
test("already merged intake can retire even when omitted prerequisite deployment evidence is unavailable", async () => {
  ...
  assert.equal(f.issue.state, "closed");
  assert.deepEqual(
    f.issue.labels.filter((label) => label.startsWith("reason:")),
    ["reason:already-merged"]
  );
  const observation = f.state().tickets[1].transitions[0].observation;
  assert.equal(
    observation.checks.find((item) => item.id === "backend_services").status,
    "unknown"
  );
...
for (const [name, change] of [
  [
    "missing intake proof",
    (f) => {
      f.run.conclusion = "failure";
    }
  ],
  [
    "unverified PR source",
    (f) => {
      f.pr.headRepository = null;
    }
  ],
  [
    "changing PR state",
    (f) => {
      ...
      f.github.pullRequest = async () => ({
        ...structuredClone(f.pr),
        state: ++reads % 2 ? "OPEN" : "MERGED"
      });
    }
  ],
  [
    "failed final PR read",
    (f) => {
      let reads = 0;
      f.github.pullRequest = async () => {
        if (++reads > 1) throw new Error("PR read unavailable");
  ...
  test(`already-merged closure requires proof: ${name}`, async () => {
    ...
    assert.ok(!f.issue.labels.includes("reason:already-merged"));
    assert.equal(
      f.state().tickets[1].transitions[0].decision.status,
      "waiting"
    );
  });

for (const state of ["OPEN", "MERGED", "unavailable", "outdated"])
  test(`combined request preserves outdated precedence and requires every PR for merged closure: companion ${state}`, async () => {
    const f = fixture();
    f.pr.state = "MERGED";
    f.request.release_parts.push({
      ...
      depends_on: ["backend"],
      pull_requests: [
        { number: 11, branch: "feature/frontend", commit: "f".repeat(40) }
      ]
    });
    saveFixtureRequest(f);
    f.github.pullRequest = async (repository) => {
      ...
      return {
        ...structuredClone(f.pr),
        number: 11,
        state: state === "outdated" ? "OPEN" : state,
        headRefOid: (state === "outdated" ? "e" : "f").repeat(40),
        headRefName: "feature/frontend",
    ...
    assert.equal(
      ...
      closed ? "closed" : "action-needed"
    );
    if (state === "outdated")
      assert.deepEqual(report.requests[0].reasons, ["outdated-commit"]);
    else assert.ok(f.issue.labels.includes("reason:already-merged"));
    if (!closed)
      assert.match(
        f.comments[0].body,
        /will not execute only part of this request/
      );
  });
...
test("a lost already-merged close response resumes without a duplicate decision", async () => {
  ...
  let failed = false;
  f.after = async (call) => {
    if (!failed && call.method === "PATCH" && call.body.state === "closed") {
      failed = true;
...
test("new intake keeps its accepted receipt when assignment is unavailable", async () => {
  ...
  assert.equal(f.comments.length, 1);
  assert.ok(saved.presentation.warnings.length);
  assert.match(f.comments[0].body, /accepted request is preserved/);
});
...
test("an uncertain initial comment POST is bound by workflow proof and recovered without duplication", async () => {
  ...
  const saved = await saveOrganizedReleaseRequestIssue(intake(f));
  assert.equal(saved.presentation.comment_id, null);
  assert.equal(f.comments.length, 1);
  f.after = async () => {};
  f.result.inbox_presentation = saved.presentation;
  await processOne(f);
  assert.equal(f.comments.length, 1);
  assert.equal(f.state().tickets[1].comment.id, f.comments[0].id);
});

test("invalid dependency scope names the submitter while missing prerequisites name maintainers", async () => {
  for (const [units, reason, status] of [
    [["api", "typo"], "invalid-dependencies", "action-needed"],
    [["api"], "prerequisite-unverified", "waiting"]
  ]) {
    const f = fixture();
    // Model a new accepted request, not an edit made after verification.
    f.request.release_parts[0].deploy_units = units;
    f.result.request = structuredClone(f.request);
    const { buildReleaseRequestIssueBody, releaseRequestChecksum } =
      await import("../../../packages/release-request/src/inbox-issue.mjs");
    f.issue.body = buildReleaseRequestIssueBody({
      ...intake(f),
      checksum: releaseRequestChecksum(f.request)
    });
    await processOne(f);
    assert.ok(f.issue.labels.includes(`reason:${reason}`));
    assert.ok(f.issue.labels.includes(`status:${status}`));
  }
});
...
test("overlap is explained without choosing the newest or replacing either request", async () => {
  const f = fixture();
  const { inspectIssue } = await import("../src/inbox-reader.mjs");
  ...
  assert.equal(f.issue.state, "open");
  assert.ok(!f.issue.labels.includes("reason:replaced"));
  assert.match(f.comments[0].body, /issue\\_numbers/);
});
...
test("failed and pending checks have different owners; resolved reasons disappear while history remains", async () => {
  ...
  assert.ok(f.issue.labels.includes("reason:checks-failed"));
  ...
  f.required.conclusion = "SUCCESS";
  f.required.status = "COMPLETED";
  await processOne(f);
  assert.ok(!f.issue.labels.includes("reason:checks-pending"));
  assert.equal(f.state().tickets[1].transitions.length, 3);
  assert.equal(f.comments.length, 1);
});

test("assignment failure keeps the accepted request and supplies verified-identity lookup", async () => {
  const f = fixture();
  f.eligible = [{ login: f.actor.login, id: 999 }];
  const result = await processOne(f);
  assert.equal(result.requests[0].assignment, "unavailable");
  assert.equal(f.issue.assignees.length, 0);
  assert.equal(
    f.calls.some(
      (call) => call.method === "POST" && call.path.endsWith("/assignees")
    ),
    false
  );
  ...
  const output = [];
  assert.equal(
    await runCli(["--json", "--submitter", "trusted-user"], {
      get: f.get,
...
test("a lost comment response resumes the same scope and comment without a duplicate decision", async () => {
  ...
  const state = f.state(),
    run = state.lock.run_id;
  assert.equal(state.tickets[1].applied, null);
  assert.equal(f.comments.length, 1);
  ...
  assert.equal(f.comments.length, 1);
  assert.equal(f.state().tickets[1].transitions.length, 1);
  assert.deepEqual(f.issues[1].labels, ["user-note"]);
  assert.equal(f.state().lock, null);
});
...
test("a lost close response resumes and verifies the explicit test disposition", async () => {
  ...
  await assert.rejects(
    processOne(f, { closeTest: true }),
    /Close response lost/
  );
  assert.equal(f.issue.state, "closed");
  await processInbox({ ...f, resume: f.state().lock.run_id });
  assert.ok(f.issue.labels.includes("reason:test"));
  assert.equal(f.state().tickets[1].transitions.length, 1);
  assert.equal(f.state().lock, null);
  const writes = issueWrites(f).length;
  await processOne(f);
...
test("test labels or titles are not closure authority and another operator cannot retire someone else's test", async () => {
  const f = fixture();
  f.issue.title = "TEST CANCELLED COMPLETED";
  f.issue.labels.push("reason:test", "status:completed");
  await processOne(f);
  assert.equal(f.issue.state, "open");
  assert.ok(!f.issue.labels.includes("reason:test"));
  await assert.rejects(
    processOne(f, {
      closeTest: true,
      identity: async () => ({ login: "other", id: "999" })
    }),
    /operator's verified request/
  );
  assert.equal(f.issue.state, "open");
});
...
test("receipt edits and corrupted decision history fail closed", async () => {
  ...
  await processOne(f);
  const state = f.state();
  state.tickets[1].transitions[0].decision.status = "completed";
  assert.throws(() => validateJournal(state), /Broken decision history/);
  const binding = f.state();
  binding.tickets[1].submitter.id = "999";
  assert.throws(() => validateJournal(binding), /identity differs/);
  const active = f.state();
  active.tickets[1].execution_owner = "future-worker";
  assert.throws(
    () => validateJournal(active),
    /unsupported ticket history\/ownership/
  );
  const writes = issueWrites(f).length;
  f.issue.body += "\nEdited";
...
test("two machines cannot both acquire the journal; a stale writer cannot pass its guard", async () => {
  ...
  assert.equal(
    results.filter((value) => value.status === "rejected").length,
    1
  );
...
test("write adapter refuses arbitrary repositories, receipt edits, force pushes, and unknown label deletion", async () => {
  ...
  for (const call of [
    ...
    },
    {
      ...
      body: { sha: "a".repeat(40), force: true }
    },
...
test("write adapter uses fixed gh arguments and stdin JSON; identity requires repository write permission", async () => {
  ...
  const client = createCoordinatorGitHub({
    execute: async (args, body) => {
      ...
      if (args[5] === "repos/6529-Collections/6529-release-coordinator")
  ...
  await client.request({
    method: "PATCH",
    path: "/issues/1",
    body: { title: "Literal $(no shell) `text`" }
  });
  assert.deepEqual(calls.at(-1).args.slice(-2), ["--input", "-"]);
  assert.equal(calls.at(-1).body.title, "Literal $(no shell) `text`");
});
...
test("processing help and bad options never contact GitHub", async () => {
  ...
  for (const args of [
    ...
    assert.equal(await runInboxRunCli(args, opts), 2);
});
