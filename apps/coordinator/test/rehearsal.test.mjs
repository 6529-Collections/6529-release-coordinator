...
import { readdir, mkdir, writeFile, readFile, rm, symlink } from "node:fs/promises";
...
test("MR-01/MR-20: real Git clean merge, repeatable trees, unchanged source, cleanup and local report", async t => {
  ...
  assert.equal(a.status, "pass"); assert.equal(rehearsalExitCode(a), 0);
  assert.equal(a.release_authorized, false); assert.equal(a.cleanup.status, "removed");
  ...
  assert.deepEqual((await readdir(f.directory)).filter(name => name.startsWith("6529-rehearsal-")), []);
...
test("MR-02: two compatible PRs both appear in the combined tree", async t => {
  ...
  const other = await f.branch("frontend", "feature/other", { "other.txt": "second\n" });
  input.repositories[0].pull_requests.push(other);
  let workspace;
  let lastMerge;
  const report = await f.run(input, { createGit: async options => {
    ...
    workspace.repository = async repo => {
      ...
      local.merge = async (...args) => { lastMerge = await merge(...args); return lastMerge; };
      return local;
    };
    const cleanup = workspace.cleanup;
    workspace.cleanup = async () => {
      const gitdir = path.join(workspace.directory, "repository-1.git");
      assert.equal(await f.git(gitdir, ["show", `${lastMerge.commit}:feature.txt`]), "new");
      assert.equal(await f.git(gitdir, ["show", `${lastMerge.commit}:other.txt`]), "second");
      await cleanup();
    };
    return workspace;
  } });
  assert.equal(report.status, "pass");
  assert.deepEqual(report.repositories[0].merges.map(m => m.head_commit), [pr.commit, other.commit]);
  assert.equal(report.repositories[0].merges[1].base_commit, report.repositories[0].merges[0].commit);
});

test("MR-03/MR-05: exact destination matters even when GitHub's PR base is clean", async t => {
  const { f, input } = await single(t, "frontend", { "shared.txt": "PR change\n" });
  const target = await f.branch("frontend", "rehearsal-target", { "shared.txt": "destination change\n" });
  input.repositories[0].destination = { branch: target.branch, commit: target.commit };
  const report = await f.run(input);
  assert.equal(report.status, "blocked");
  assert.deepEqual(report.repositories[0].merges[0].conflicts, ["shared.txt"]);
  assert.equal(checks(report).find(c => c.id === "destination_gate").status, "unknown");
});

test("MR-04: PRs that merge separately can conflict together", async t => {
  const { f, pr, input } = await single(t, "frontend", { "shared.txt": "one\n" });
  const other = await f.branch("frontend", "feature/conflict", { "shared.txt": "two\n" });
  assert.equal((await f.run(input)).status, "pass");
  assert.equal((await f.run(f.manifest([{ role: "frontend", pulls: [other] }]))).status, "pass");
  input.repositories[0].pull_requests = [pr, other];
  const report = await f.run(input);
  assert.equal(report.status, "blocked"); assert.equal(rehearsalExitCode(report), 1);
...
test("MR-06/MR-07: separate frontend/backend trees with whole-request outcome", async t => {
  ...
  const backend = await f.branch("backend", "feature/backend", { "shared.txt": "backend one\n" });
  const input = f.manifest([{ role: "backend", pulls: [backend] }, { role: "frontend", pulls: [pr], depends_on: ["backend"] }]);
  let report = await f.run(input);
  assert.equal(report.status, "pass"); assert.equal(report.repositories.filter(r => r.final_tree).length, 2);
  assert.deepEqual(checks(report).find(c => c.id === "combined_services").evidence.order, ["backend/dbMigrationsLoop", "backend/api", "frontend/frontend"]);
  input.repositories[0].pull_requests.push(await f.branch("backend", "feature/backend-conflict", { "shared.txt": "backend two\n" }));
  report = await f.run(input);
  assert.equal(report.status, "blocked"); assert.ok(report.repositories[1].final_tree);
});
...
test("MR-08: changed requested head or destination is not silently substituted", async t => {
  ...
  f.repositories.frontend.pulls.get(1).commit = await f.git(f.repositories.frontend.cwd, ["rev-parse", "HEAD"]);
  const original = f.github.pullRequest;
  f.github.pullRequest = async (...args) => ({ ...await original(...args), headRefOid: "e".repeat(40) });
  let calls = 0;
  const report = await f.run(input, { createGit: async () => { calls++; throw new Error("must not run"); } });
  assert.equal(report.status, "blocked"); assert.equal(calls, 0);
  assert.equal(checks(report).find(c => c.id === "requested_code").status, "blocked");
});

for (const change of ["head", "destination", "checks", "review", "state", "unavailable"]) test(`MR-09/MR-10: final ${change} changes invalidate observations`, async t => {
  const { f, input } = await single(t);
  let reads = 0;
  const original = change === "destination" ? f.github.destination : f.github.pullRequest;
  const method = change === "destination" ? "destination" : "pullRequest";
  f.github[method] = async (...args) => {
    const value = await original(...args);
    if (++reads > 1) {
      if (change === "unavailable") throw new Error("secret token must not appear");
...
for (const condition of ["failed", "pending", "missing", "review", "draft", "closed", "merged"]) test(`MR-11/MR-12/MR-13: ${condition} never passes`, async t => {
  const { f, input } = await single(t);
  const original = f.github.pullRequest;
  f.github.pullRequest = async (...args) => {
    ...
    if (condition === "missing") { value.checks = []; value.mergeStateStatus = "BLOCKED"; }
  ...
  if (["failed", "pending", "missing", "review"].includes(condition)) assert.ok(report.repositories[0].final_tree);
...
test("MR-14/MR-21: manifest/profile validation rejects unsafe or mixed inputs before tools", async t => {
  const { f, input } = await single(t);
  for (const mutate of [
    v => { v.profile = "real"; }, v => { v.source = "inbox"; }, v => { v.repositories[0].repository = "6529seize-frontend"; },
    v => { v.repositories[0].destination.commit = "short"; }, v => { delete v.repositories[0].destination; },
    v => { v.repositories[0].pull_requests[0].branch = "--upload-pack=bad"; },
    v => { v.repositories[0].pull_requests.push(v.repositories[0].pull_requests[0]); },
    v => { v.repositories[0].depends_on = ["backend"]; }, v => { v.repositories[0].role = "other"; },
    v => { v.repositories[0].destination.branch = "a/../b"; }, v => { v.repositories[0].destination.branch = "a.lock"; }
  ]) {
...
test("MR-15: unavailable objects remain unknown and preserve cleanup", async t => {
  ...
  const original = f.github.pullRequest;
  f.github.pullRequest = async (...args) => ({ ...await original(...args), headRefOid: missing });
  const report = await f.run(input);
...
test("MR-16: service validation uses the exact combined catalog", async t => {
  const f = await rehearsalFixture(t);
  const a = sampleCatalog(); a.services[0].name = "api-v2";
  const b = sampleCatalog(); b.services[1].allowed_environments.push("prod");
  const prs = [await f.branch("backend", "feature/catalog-a", { "src/config/deploy-services.json": `${JSON.stringify(a, null, 2)}\n` }),
    await f.branch("backend", "feature/catalog-b", { "src/config/deploy-services.json": `${JSON.stringify(b, null, 2)}\n` })];
  const report = await f.run(f.manifest([{ role: "backend", pulls: prs, deploy_units: ["api-v2", "dbMigrationsLoop"] }]));
  assert.equal(report.status, "pass");
  const catalog = report.repositories[0].catalog;
  for (const pr of prs) assert.notEqual(catalog.blob_sha, await f.git(f.repositories.backend.cwd, ["rev-parse", `${pr.commit}:src/config/deploy-services.json`]));
});

for (const condition of ["unknown", "omitted", "cycle"]) test(`MR-17: ${condition} service prerequisites`, async t => {
  ...
  if (condition === "cycle") repo.deploy_dependencies = [{ before: "api", after: "dbMigrationsLoop" }];
  const report = await f.run(input);
  assert.equal(report.status, condition === "omitted" ? "unknown" : "blocked");
});
...
test("MR-18: cleanup failure reports the owned directory and prevents success", async t => {
  ...
  const report = await f.run(input, { createGit: options => createRehearsalGit({ ...options, temporaryRoot: f.directory,
    fixtureRemotes: { frontend: f.repositories.frontend.cwd }, remove: async () => { throw new Error("denied"); } }) });
...
test("MR-18: failed report writes happen after cleanup and are operational errors", async t => {
  ...
  assert.equal(exit, 2); assert.equal(saved.cleanup.status, "removed"); assert.doesNotMatch(output, /private token/u);
});

test("MR-18/MR-19: timeout, interrupt, output limits and credential-safe errors", async () => {
  await assert.rejects(runRehearsalProcess(process.execPath, ["-e", "setInterval(() => {}, 1000)"], { timeout: 25 }), { code: "timeout" });
  const controller = new AbortController();
  const promise = runRehearsalProcess(process.execPath, ["-e", "setInterval(() => {}, 1000)"], { signal: controller.signal });
  controller.abort(); await assert.rejects(promise, { code: "interrupted" });
  await assert.rejects(runRehearsalProcess(process.execPath, ["-e", "process.stdout.write('x'.repeat(10000))"], { maxOutput: 100 }), { code: "output_limit" });
  await assert.rejects(runRehearsalProcess(process.execPath, ["-e", "process.stderr.write('SECRET_TOKEN'); process.exit(1)" ]), error => error.code === "tool_failed" && !String(error).includes("SECRET_TOKEN"));
});

test("MR-19: inherited Git hooks/config cannot run, attribute rules are explicit unsupported evidence", async t => {
  const { f, input } = await single(t, "frontend", { ".gitattributes": "* merge=evil filter=evil\n" });
  const marker = path.join(f.directory, "executed");
  const config = path.join(f.directory, "evil-config");
  await writeFile(config, `[merge "evil"]\n driver = touch ${marker}\n[filter "evil"]\n smudge = touch ${marker}\n`);
  ...
  } finally { if (before === undefined) delete process.env.GIT_CONFIG_GLOBAL; else process.env.GIT_CONFIG_GLOBAL = before; }
});
...
test("MR-19: bounded file reads and report symlink refusal", async t => {
  ...
  await assert.rejects(readRehearsalManifest(filename), { code: "invalid_manifest" });
  const destination = path.join(f.directory, "elsewhere"); await mkdir(destination);
  await symlink(destination, path.join(f.directory, ".release-coordinator"));
  await assert.rejects(saveRehearsalReport({ profile: "sandbox", run_id: "11111111-1111-4111-8111-111111111111" }, f.directory), { code: "unsafe_report_path" });
});

test("MR-18: interrupted merge removes its already-created workspace", async t => {
  const { f, input } = await single(t);
  const controller = new AbortController();
  const report = await f.run(input, { signal: controller.signal, createGit: async options => {
    ...
    session.repository = async repo => {
      ...
      workspace.merge = async (...args) => { controller.abort(); return merge(...args); };
      return workspace;
    };
    return session;
  } });
...
test("MR-18: storage limits also clean up setup failures", async t => {
  const { f, input } = await single(t);
  const report = await f.run(input, { createGit: options => createRehearsalGit({ ...options, temporaryRoot: f.directory, maxStorageBytes: 1 }) });
  assert.equal(report.status, "unknown");
...
test("MR-12: an optional sample check cannot replace the profile's required GitHub enforcement", async t => {
  ...
  const original = f.github.pullRequest;
  f.github.pullRequest = async (...args) => {
    const pr = await original(...args); pr.checks[0].isRequired = false; return pr;
  };
  const report = await f.run(input);
