...
import {
  ...
  inspectIssue,
  readInbox
} from "../src/inbox-reader.mjs";
import { runCli } from "../src/cli.mjs";

const api = `repos/${COORDINATOR_REPOSITORY}`;
const web = `https://github.com/${COORDINATOR_REPOSITORY}`;
const issuesPath = (page) =>
  `${api}/issues?state=open&labels=release-request&sort=created&direction=asc&per_page=100&page=${page}`;
const jobsPath = (page = 1, attempt = 1) =>
  `${api}/actions/runs/123/attempts/${attempt}/jobs?per_page=100&page=${page}`;
...
function fixture() {
    ...
    release_parts: [
      {
        ...
        pull_requests: [
          { number: 1977, branch: "codex/backend", commit: "a".repeat(40) }
        ],
        depends_on: [],
        deploy_units: ["api", "worker"],
      ...
      },
      {
        ...
        pull_requests: [
          { number: 3907, branch: "codex/frontend", commit: "b".repeat(40) }
        ],
        depends_on: ["backend"]
      }
    ]
  ...
  const run = {
    id: 123,
    repository: { full_name: COORDINATOR_REPOSITORY },
  ...
  const result = {
    ...
    inbox_issue_number: 10,
    inbox_issue_url: `${web}/issues/10`,
    github: {
      actor: "trusted-user",
      actor_id: "456",
      workflow_run_id: "123",
...
test("verifies both product parts and distinguishes supplied requester from GitHub actor", async () => {
  ...
  assert.deepEqual(report.requests[0].github_actor, {
    login: "trusted-user",
    id: "456"
  });
  ...
  for (const value of [
    ...
    "Requested by (supplied text)",
    "Verified GitHub actor: @trusted-user",
    "PR #1977",
    "PR #3907",
    "a".repeat(40),
    "b".repeat(40),
    "Backend units: api, worker",
    "api -> worker",
    "depends on: backend",
    "Current PR readiness and permission to release are not checked."
  ]) {
...
for (const [name, mutate, expected] of [
  ...
  [
    "malformed JSON",
    (d) => {
      d.issue.body = d.issue.body.replace('"schema_version":', "broken:");
    },
    /not valid JSON/
  ],
  [
    "schema violation",
    (d) => {
      d.issue.body = d.issue.body.replace(
        '"target": "staging"',
        '"target": "moon"'
      );
    },
  ...
  [
    "duplicate workflow table row",
    (d) => {
      d.issue.body = d.issue.body.replace(
        "| Workflow |",
        `| Workflow | ${d.run.html_url} |\n| Workflow |`
      );
    },
    /repeats the Workflow/
  ],
  [
    "table target conflict",
    (d) => {
      d.issue.body = d.issue.body.replace(
        "| Target | `staging` |",
        "| Target | `production` |"
  ...
  [
    "workflow URL suffix injection",
    (d) => {
      d.issue.body = d.issue.body.replace(
        d.run.html_url,
        `${d.run.html_url}/../../issues`
      );
    },
    /workflow link/
  ]
...
for (const [name, mutate] of [
  [
    "another workflow",
    (d) => {
      d.run.path = ".github/workflows/fake.yml";
    }
  ],
  [
    "non-main branch",
    (d) => {
      d.run.head_branch = "codex/fake";
    }
  ],
  [
    "another event",
  ...
  [
    "foreign repository",
    (d) => {
      d.run.repository.full_name = "other/repo";
    }
  ],
  [
    "fork source",
    (d) => {
      d.run.head_repository.full_name = "other/repo";
    }
  ],
  [
    "another request's run",
    (d) => {
      d.run.display_title = "Release request something-else";
    }
  ],
  [
    "edited actor",
    (d) => {
      d.run.actor.login = "different-user";
    }
  ],
  [
    "edited actor ID",
  ...
  [
    "wrong result ID",
    (d) => {
      d.result.request_id = "33333333-3333-4333-8333-333333333333";
    }
  ],
  [
    "wrong result URL",
    (d) => {
      d.result.inbox_issue_url = "https://example.org/issues/10";
    }
  ],
  [
    "wrong result actor",
    (d) => {
      d.result.github.actor = "different-user";
    }
  ],
  [
    "wrong result run",
    (d) => {
      d.result.github.workflow_run_id = "999";
    }
  ],
  [
    "self-consistent payload tampering",
    (d) => {
      const checksum = releaseRequestChecksum(d.request);
      d.request.database_change = "yes";
      d.issue.body = d.issue.body
        .replace('"database_change": "no"', '"database_change": "yes"')
...
test("unavailable, failed, ambiguous or unreadable proof stays unverified", async (t) => {
  for (const [name, configure] of [
    [
      "missing run",
      (d, o) => o.set(`${api}/actions/runs/123`, new Error("HTTP 404"))
    ],
    [
      "failed run",
      (d) => {
    ...
    [
      "duplicate save job",
      (d, o) => o.set(jobsPath(), { jobs: [d.job, { ...d.job, id: 790 }] })
    ],
    [
      "expired logs",
      (d, o) => o.set(`${api}/actions/jobs/789/logs`, new Error("Logs expired"))
    ],
    [
      "missing marker",
      (d, o) => o.set(`${api}/actions/jobs/789/logs`, "No saved result\n")
    ],
    [
      "duplicate result",
      (d, o) =>
        o.set(`${api}/actions/jobs/789/logs`, marker(d.result).repeat(2))
    ],
    [
      "unreadable result",
      (d, o) =>
        o.set(
          `${api}/actions/jobs/789/logs`,
          "2026-09-08T10:00:02.123Z RELEASE_REQUEST_RESULT=bm90LWpzb24\n"
        )
    ]
  ]) {
...
test("quoted or echoed markers cannot stand in for actual workflow output", async () => {
  const data = fixture();
  const encoded = Buffer.from(JSON.stringify(data.result)).toString(
    "base64url"
  );
  ...
  assert.equal(
    (await inspectIssue(data.issue, github(data, overrides))).status,
    "unverified"
  );
...
test("reads every Issue page and excludes closed tests, PRs and missing labels", async () => {
  ...
  const firstPage = Array.from({ length: 100 }, (_, index) => ({
    ...data.issue,
    number: index + 100,
    body: "Malformed pending request"
  }));
  const excluded = [
    {
      ...data.issue,
      number: 12,
      state: "closed",
      title: "DELIVERY TEST - NO RELEASE"
    },
  ...
  const client = github(
    data,
    new Map([
      [issuesPath(1), firstPage],
      [issuesPath(2), [...excluded, data.issue]]
    ])
  );
  ...
  assert.ok(
    !report.requests.some((entry) =>
      [12, 13, 14, 15].includes(entry.issue_number)
    )
  );
...
test("reads paginated jobs before identifying the save job", async () => {
  ...
  const client = github(
    data,
    new Map([
      [jobsPath(), { jobs: unrelated }],
      [jobsPath(2), { jobs: [data.job] }]
    ])
  );
...
test("duplicate request IDs are reported together rather than choosing a release", async () => {
  ...
  const client = github(
    data,
    new Map([[issuesPath(1), [data.issue, { ...data.issue, number: 11 }]]])
  );
  const report = await readInbox(client);
  assert.deepEqual(report.counts, {
    pending: 2,
...
test("listing errors, unexpected responses and repeating pages never report an empty inbox", async () => {
  ...
  for (const overrides of [
    ...
    new Map([
      [issuesPath(1), page],
      [issuesPath(2), new Error("Rate limit exceeded")]
    ]),
    new Map([
      [issuesPath(1), page],
      [issuesPath(2), page]
...
test("all reads use gh GET against the fixed repository without a shell or request bodies", async () => {
  ...
  const get = createGitHubReader({
    execute: async (file, args, options) => {
      ...
      for (const flag of ["--field", "-f", "-F", "--input"])
  ...
  for (const endpoint of [
    "https://example.org",
    `${api}/issues/10`,
    `${api}/actions/runs/123/cancel`,
    `${api}/actions/runs/123?method=POST`,
...
test("gh execution and JSON failures are explicit and do not expose raw logs or stderr", async () => {
  for (const execute of [
    async () => {
      throw Object.assign(new Error("secret token in stderr"), { code: 1 });
    },
    ...
    await assert.rejects(
      createGitHubReader({ execute })(issuesPath(1)),
      (error) => {
...
test("human report escapes controls in editable titles and supplied requester names", async () => {
  ...
  const output = formatReport(report);
  // eslint-disable-next-line no-control-regex -- Verify that terminal controls cannot reach human output.
...
test("CLI exit codes distinguish verified, invalid, unverified, and failed inbox reads", async () => {
  for (const [overrides, code] of [
    ...
    let output = "";
    let errors = "";
    const actual = await runCli(["--json"], {
      ...github(fixture(), overrides),
      stdout: (value) => {
        output += value;
      },
      stderr: (value) => {
...
test("help and invalid options never read GitHub", async () => {
  const client = {
    get: () => assert.fail("GitHub must not be contacted"),
    stdout: () => {},
    stderr: () => {}
  };
...
test("submitter filtering accepts letters, digits, internal hyphens, and case differences", async () => {
  for (const login of ["trusted-user", "TRUSTED-USER", "a", "User42"]) {
    ...
    let output = "";
    assert.equal(
      await runCli(["--submitter", login, "--json"], {
        ...client,
        stdout: (value) => {
          output += value;
        },
        stderr: () => assert.fail("Valid login rejected")
      }),
      0
    );
    const report = JSON.parse(output);
    ...
    assert.equal(
      ...
      login.toLowerCase() === "trusted-user" ? 1 : 0
    );
