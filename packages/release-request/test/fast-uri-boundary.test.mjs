...
const packageDirectory = await realpath(
  process.env.RELEASE_REQUEST_TEST_PACKAGE_DIR
  ...
  return {
    ...
    async runGh(args, options = {}) {
      ...
      assert.deepEqual(
        args,
        expected[step - 1],
        "Request text must not change GitHub routing or arguments"
      );
      let stdout = "";
      if (step === 2) {
...
test("release-request input cannot reach fast-uri address normalization", async (t) => {
  ...
  const forbidExternal = () => {
    externalAttempts += 1;
    throw new Error(
      "This local assessment forbids real network calls and child processes"
    );
  };
  ...
  const direct = payloads.map((input) => {
    try {
      const output = input.startsWith("//")
        ? uri.resolve("http://trusted.example/base", input)
        : uri.normalize(input);
      return { input, output };
    } catch (error) {
  ...
  assert.equal(
    uriCalls.length,
    payloads.length,
    "The tracer must observe the direct probes"
  );
  t.diagnostic(
    JSON.stringify({
      packageDirectory,
      fastUriVersion: uriVersion,
      directProbes: direct
    })
  );
  uriCalls.length = 0;
  ...
  const api = await import(
    pathToFileURL(path.join(packageDirectory, "src/index.mjs"))
  );
  const transport = await import(
    pathToFileURL(path.join(packageDirectory, "src/github-submission.mjs"))
  );
  assert.ok(
    uriCalls.length > 0,
    "The tracer must observe Ajv compiling the bundled schema"
  );
  ...
  const fixture = JSON.parse(
    await readFile(
      path.join(packageDirectory, "release-request.example.json"),
      "utf8"
    )
  );
  fixture.release_parts[0].deploy_units = ["dbMigrationsLoop", "api"];
  fixture.release_parts[0].deploy_dependencies = [
    { before: "dbMigrationsLoop", after: "api" }
  ];
    ...
    for (const input of payloads) {
      for (const location of locations) {
        ...
        assert.deepEqual(
          request,
          before,
          "Validation must not normalize or alter input"
        );
        assert.equal(
          uriCalls.length,
          0,
          `fast-uri reached from ${location.join("/")}`
        );
      }
    }
  });
  ...
  await t.test(
    "request fields cannot install a schema or remote reference",
    () => {
      for (const input of payloads) {
        for (const field of [
          "$schema",
          ...
          for (const location of [
            [],
            ["release_parts", "0"],
            ["release_parts", "0", "pull_requests", "0"]
          ]) {
  ...
  await t.test(
    "accepted free text survives local creation and simulated GitHub submission unchanged",
    async () => {
      const directory = await mkdtemp(
        path.join(tmpdir(), "6529-fast-uri-boundary-")
      );
      try {
        for (const input of payloads) {
          ...
          const result = await api.submitReleaseRequestRun({
            ...
            submitRequest: ({ request }) =>
              transport.submitReleaseRequestToGitHub({
                request,
                runGh: github.runGh
              })
          });
          ...
          const saved = JSON.parse(
            await readFile(path.join(directory, result.requestPath), "utf8")
          );
